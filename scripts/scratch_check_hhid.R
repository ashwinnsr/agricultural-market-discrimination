...
mh1 <- d1 %>% filter(as.character(STATE) == "27") %>% head(5)
mh2 <- d2 %>% filter(as.character(STATE) == "27") %>% head(5)

cat("Visit 1 MH IDs:\n")
print(mh1 %>% select(HHID, FSU_SLNO, HH_NO, VISITNO, SSS, SAMPLE))

cat("\nVisit 2 MH IDs:\n")
print(mh2 %>% select(HHID, FSU_SLNO, HH_NO, VISITNO, SSS, SAMPLE))
